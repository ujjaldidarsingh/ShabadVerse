...
paths=['/health/ready','/api/graph/init','/about','/api/topics/Naam','/api/topics/Naam/review',
       ...
       '/api/graph/neighbors/970?source=ak-only&max_neighbors=6',
       '/api/graph/neighbors/970?source=ak-only&match=line&max_neighbors=6']
for path in paths:
    response=client.get(path)
    assert response.status_code==200,(path,response.status_code,response.data[:300])
    data=response.get_json(silent=True)
    if 'neighbors' in path:
        assert data['source_mode']=='ak-only'
        assert all(n['is_amrit_keertan'] for group in data['by_tag'].values() for n in group)
