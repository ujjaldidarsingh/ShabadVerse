...
(async()=>{
 ...
 try {for(const [label,width,height] of [['phone',390,844],['tablet',820,1180],['desktop',1440,1000]]){
  ...
  await context.addInitScript(()=>localStorage.setItem('shabadverse_view','list'));
  ...
  await page.locator('#sourceModeSelect').selectOption('ak-only');await page.waitForFunction(()=>!State.expanding && State.lastNeighbors?.data.source_mode==='ak-only');
  assert.equal(await page.evaluate(()=>Object.values(State.lastNeighbors.data.by_tag).flat().every(n=>n.is_amrit_keertan)),true);
  await page.locator('#sourceModeSelect').selectOption('all');await page.waitForFunction(()=>!State.expanding && State.lastNeighbors?.data.source_mode==='all');
  ...
  await page.locator('#topicSource').selectOption('ak-only');await page.waitForFunction(()=>Topic.source==='ak-only' && document.querySelector('#topicRandom'));
  ...
  assert.equal(await page.evaluate(()=>State.metadata[State.centerNode].is_amrit_keertan),true);
  ...
  await page.locator('#sourceModeSelect').selectOption('prefer-ak');await page.waitForFunction(()=>!State.expanding&&State.lastNeighbors.data.source_mode==='prefer-ak');
  ...
  const controls=await page.locator('#sourceModeSelect').boundingBox();assert.ok(controls.y+controls.height<=height-(label==='phone'?52:0),'source controls visible above navigation');
