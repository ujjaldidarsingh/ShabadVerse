...
const Topic = {all: null, tag: null, offset: 0, generation: 0, recent: {}};
...
async function selectTag(tag, offset = 0) {
    ...
    try {
        const data = await API.get(`/api/topics/${encodeURIComponent(tag)}?offset=${offset}`);
        ...
        detail.innerHTML = `<header class="topic-heading"><div><span class="eyebrow">Explore a topic</span><h2>${escapeHtml(tag)}</h2></div><button type="button" id="topicRandom" ${data.total ? '' : 'disabled'}>Random starting shabad</button></header>
            <p>${escapeHtml(t.description)}</p><p class="topic-counts"><strong>${t.count.toLocaleString()}</strong> shabads · ${t.lexical.toLocaleString()} lexical · ${t.inferred.toLocaleString()} inferred</p>
            ...
            <div class="topic-tools"><button type="button" id="topicReview">Review assignment examples</button><span>${data.total ? `${offset+1}–${Math.min(offset+20,data.total)} of ${data.total.toLocaleString()}` : 'No shabads in this topic'}</span></div>
            <div id="topicReviewSamples"></div><div class="topic-results">${data.shabads.map(s => `<article class="topic-row"><button type="button" class="topic-open" data-open="${s.id}"><span lang="pa-Guru">${escapeHtml(s.gurmukhi || s.title)}</span><small>${escapeHtml([s.raag,s.writer,'Ang '+s.ang].filter(Boolean).join(' · '))}</small></button><button type="button" data-read="${s.id}">Read</button></article>`).join('')}</div>
        ...
        document.getElementById('topicRandom').onclick = () => randomTopic(tag);
...
async function randomTopic(tag) {
    const key = tag;
    ...
    const generation = ++Topic.generation;
    try {
        const data = await API.get(`/api/topics/${encodeURIComponent(tag)}/random?exclude=${recent.join(',')}`);
        if (generation !== Topic.generation) return;
        Topic.recent[key] = [...recent.slice(-29),data.shabad.id];
        State.metadata[data.shabad.id] = {...State.metadata[data.shabad.id],...data.shabad};
        startFromTopic(tag,data.shabad.id);
    } catch (error) { showToast('No starting shabad available in this topic.'); }
    finally { if (button) button.disabled = false; }
}
function startFromTopic(tag, sid) {
    closeTagBrowser();
    switchToTab('explore');
    delete State.selectedTuk[sid];
    State.startingTopic = {tag};
    ...
    document.getElementById('topicAnother').onclick = () => randomTopic(tag);
    expandShabad(sid);
}
function surpriseMe() {
